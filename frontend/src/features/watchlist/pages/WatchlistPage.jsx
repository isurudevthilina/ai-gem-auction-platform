...
import { useGetWatchlist, useRemoveFromWatchlist, useMoveToFolder, useRemoveOlderThan } from '../hooks/useWatchlist';
...
const WatchlistPage = () => {
    ...
    const cleanupMutation = useRemoveOlderThan();
        ...
        <div style={{ minHeight: '100vh', background: C.bg, fontFamily: BODY }}>
            <main style={{ maxWidth: 1100, margin: '0 auto', padding: '44px 24px 80px' }}>
                ...
                <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center', margin: '0 0 32px' }}>
                        <p style={{ margin: 0, fontFamily: BODY, fontSize: '0.92rem', color: C.muted }}>
                            {totalCount} saved gem{totalCount !== 1 ? 's' : ''}
                        </p>
                        <div style={{ marginLeft: 'auto', display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center' }}>
                            <span style={{ fontFamily: BODY, fontSize: '0.8rem', color: C.faint }}>Auto-remove older than</span>
                            <select
                                value={retentionDays}
                                onChange={(e) => setRetentionDays(Number(e.target.value))}
                                style={{
                                    padding: '8px 10px', borderRadius: 10, border: `1px solid ${C.border}`,
                                    background: C.white, color: C.text, fontFamily: BODY, fontSize: '0.82rem',
                                }}
                            >
                            ...
                            <button
                                onClick={() => {
                                    if (window.confirm(`Remove watchlist items older than ${retentionDays} days?`)) {
                                ...
                                style={{
                                    ...
                                    background: C.sapphire, color: '#fff', fontFamily: BODY, fontSize: '0.82rem',
                                    fontWeight: 600, cursor: cleanupMutation.isPending ? 'wait' : 'pointer',
                                    opacity: cleanupMutation.isPending ? 0.7 : 1,
                                }}
                            >
                                {cleanupMutation.isPending ? 'Removing…' : 'Remove Old Items'}
                            </button>
                        </div>
                    </div>
                </motion.div>
