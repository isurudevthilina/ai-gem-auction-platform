...
const BidHistoryCard = ({ item, onContactSeller }) => {
    ...
    const hasValidAuctionId = UUID_V4_LIKE_REGEX.test(String(item.auction_id || ''));
        ...
        >
            ...
            <div style={{ flex: 1, padding: '16px 20px' }}>
                ...
                <div style={{
                    ...
                    cursor: hasValidAuctionId ? 'pointer' : 'default',
                }} onClick={() => hasValidAuctionId && navigate(`/auctions/${item.auction_id}`)}>
                    {gem?.title || 'Untitled'}
                </div>
            ...
            <div style={{
                ...
                {outcome === 'winning' && (
                    <>
                        ...
                        <span onClick={() => hasValidAuctionId && navigate(`/auctions/${item.auction_id}`)} style={{
                            fontFamily: BODY, fontSize: '0.78rem', color: C.sapphire,
                            cursor: hasValidAuctionId ? 'pointer' : 'default', textDecoration: 'underline', textAlign: 'center',
                        }}>View Auction</span>
                    </>
                )}
                ...
                {outcome === 'outbid' && (
                    <>
                        <button disabled={!hasValidAuctionId} onClick={() => hasValidAuctionId && navigate(`/auctions/${item.auction_id}`)} style={{
                            background: C.red, color: '#fff', border: 'none', borderRadius: 10,
                            padding: '10px 16px', fontFamily: BODY, fontSize: '0.82rem',
                            fontWeight: 600, cursor: hasValidAuctionId ? 'pointer' : 'not-allowed', opacity: hasValidAuctionId ? 1 : 0.55,
                        }}>Bid Again</button>
                ...
                {outcome === 'lost' && (
                    <>
                        <button disabled={!hasValidAuctionId} onClick={() => hasValidAuctionId && navigate(`/auctions/${item.auction_id}`)} style={{
                            ...
                            fontFamily: BODY, fontSize: '0.82rem', fontWeight: 600, cursor: hasValidAuctionId ? 'pointer' : 'not-allowed', opacity: hasValidAuctionId ? 1 : 0.55,
                        }}>View Auction</button>
                        <span style={{
                ...
                {(outcome === 'cancelled' || outcome === 'reserve_not_met' || outcome === 'ended') && (
                    <button style={{
                        ...
                        color: C.muted, borderRadius: 10, padding: '10px 16px',
                        fontFamily: BODY, fontSize: '0.82rem', fontWeight: 600,
                        cursor: hasValidAuctionId ? 'pointer' : 'not-allowed', opacity: hasValidAuctionId ? 1 : 0.55,
                    }} disabled={!hasValidAuctionId} onClick={() => hasValidAuctionId && navigate(`/auctions/${item.auction_id}`)}>
                    View Details</button>
                )}
