...
import { useMyWallet } from '../../wallet/hooks/useWallet';
...
const BidPanel = ({ auction, onBidPlaced, isExpired }) => {
    ...
    const { data: wallet, isLoading: walletLoading } = useMyWallet({ enabled: isAuthenticated });
    ...
    const needsTopUp = typeof error === 'string' && error.toLowerCase().includes('wallet balance');
    const walletBalance = Number(wallet?.available_balance || 0);
    const hasMinimumWallet = walletBalance >= MIN_AUCTION_WALLET_BALANCE;
    const isWalletGateActive = isAuthenticated && !walletLoading;
    const isBelowMinimumWallet = isWalletGateActive && !hasMinimumWallet;
    const missingWalletAmount = Math.max(0, MIN_AUCTION_WALLET_BALANCE - walletBalance);
    const minimumWalletLabel = formatPrice(MIN_AUCTION_WALLET_BALANCE);
    const holdAmountLabel = formatPrice(BID_SECURITY_HOLD);
    ...
    useEffect(() => {
        ...
        } else if (parsed < minRequired) {
            setError(`Minimum bid is ${formatPrice(minRequired)}.`);
        } else {
            setError(null);
        }
    }, [amount, minRequired, formatPrice]);
    ...
    const handleBid = async () => {
        ...
        if (isBelowMinimumWallet) {
            setError(`A minimum wallet balance of ${minimumWalletLabel} is required before joining this auction.`);
            return;
        }
        ...
        try {
            ...
            if (err?.required_min_wallet_balance) {
                setError(`A minimum wallet balance of ${formatPrice(err.required_min_wallet_balance)} is required before joining this auction.`);
            } else {
    ...
    return (
        <div style={{
            ...
            <div style={{
                border: `1px solid ${isBelowMinimumWallet ? 'rgba(185,28,28,0.22)' : 'rgba(196,137,42,0.22)'}`,
                borderRadius: 10,
                padding: '10px 12px',
                background: isBelowMinimumWallet ? 'rgba(185,28,28,0.05)' : 'rgba(196,137,42,0.08)',
                display: 'flex',
                flexDirection: 'column',
                gap: 5,
            }}>
                <div style={{ color: C.text, fontSize: '0.78rem', fontFamily: BODY, fontWeight: 700 }}>
                    Minimum wallet required to join: {minimumWalletLabel}
                </div>
                <div style={{ color: C.muted, fontSize: '0.74rem', fontFamily: BODY, fontWeight: 600 }}>
                    Security hold per current highest bidder: {holdAmountLabel}
                </div>
                {isAuthenticated && !walletLoading && (
                    <div style={{ color: isBelowMinimumWallet ? C.red : C.green, fontSize: '0.74rem', fontFamily: BODY, fontWeight: 600 }}>
                        Your wallet: {formatPrice(walletBalance)}
                        {isBelowMinimumWallet ? ` (top up ${formatPrice(missingWalletAmount)} more)` : ' (eligible to bid)'}
                    </div>
                )}
                {!isAuthenticated && (
                    <div style={{ color: C.muted, fontSize: '0.74rem', fontFamily: BODY }}>
                        Sign in and keep at least {minimumWalletLabel} in your wallet before placing a bid.
                    </div>
                )}
            </div>
            ...
            {isActive ? (
                <>
                    ...
                    <AnimatePresence>
                        ...
                        {isBelowMinimumWallet && !needsTopUp && (
                            <button
                                type="button"
                                onClick={() => navigate('/wallet/top-up')}
                                style={{
                            ...
                            >
                                Top up to {minimumWalletLabel} minimum
                            </button>
                        )}
                        {success && (
                            <motion.div
                    ...
                    <button
                        onClick={handleBid}
                        disabled={loading || isBelowMinimumWallet}
                        style={{
                            width:        '100%',
                            background:   (loading || isBelowMinimumWallet) ? C.border : C.gold,
                            color:        C.white,
                            border:       'none',
                            borderRadius: 10,
                            padding:      '14px',
                            ...
                            fontFamily:   BODY,
                            fontWeight:   800,
                            cursor:       (loading || isBelowMinimumWallet) ? 'not-allowed' : 'pointer',
                    ...
                    >
                        {loading ? 'Placing Bid…' : isBelowMinimumWallet ? `Minimum ${minimumWalletLabel} Required` : (
                            <>
