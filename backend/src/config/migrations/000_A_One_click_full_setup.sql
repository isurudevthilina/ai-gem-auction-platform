-- GemBid LK: One-Click Full Setup for a NEW Supabase database
-- Run this file alone in Supabase SQL Editor (new project).
-- Includes: base schema + certification body + review limits + moderation + review media + virtual wallet + second-chance + review repost sequence.

-- ==================== BEGIN 001_gembid_complete_schema.sql ====================
/*

  GemBid LK  Complete Database Schema
  Supabase (PostgreSQL)  Fully Idempotent  safe to re-run


  MANUAL SETUP REQUIRED IN SUPABASE DASHBOARD  Storage:
  ...
  Bucket 2: certificates
    ...
    - Max file size: 10MB
    - RLS: seller can upload; admin can read all; public cannot read
*/
...
-- Idempotent column additions (safe for existing installs)
...
-- Auto-update last_login_at from auth.users sign-in events
...
-- Trigger
DROP TRIGGER IF EXISTS gems_set_updated_at ON public.gems;
...
CREATE INDEX IF NOT EXISTS idx_gems_category_id  ON public.gems(category_id);
CREATE INDEX IF NOT EXISTS idx_gems_status       ON public.gems(status);
CREATE INDEX IF NOT EXISTS idx_gems_listing_type ON public.gems(listing_type);

-- RLS
ALTER TABLE public.gems ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "gems_select" ON public.gems;
...
DROP POLICY IF EXISTS "gems_delete_draft" ON public.gems;
...
CREATE INDEX IF NOT EXISTS idx_auctions_gem_id    ON public.auctions(gem_id);
...
-- Immutability rules (idempotent: drop first)
DROP RULE IF EXISTS no_update_bids ON public.bids;
...
-- INSERT is done by SECURITY DEFINER RPCs (place_bid, complete_expired_auctions, buy_now).
-- No direct insert policy for authenticated users.
...
-- Trigger
DROP TRIGGER IF EXISTS watchlist_folders_set_updated_at ON public.watchlist_folders;
...
DROP POLICY IF EXISTS "watchlist_folders_all_own" ON public.watchlist_folders;
...
-- Unique constraint (idempotent via DO block since ALTER TABLE ADD CONSTRAINT IF NOT EXISTS is PG15+)
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'watchlist_user_gem_unique'
    ) THEN
        ALTER TABLE public.watchlist ADD CONSTRAINT watchlist_user_gem_unique UNIQUE (user_id, gem_id);
    END IF;
END $$;

-- Indexes
CREATE INDEX IF NOT EXISTS idx_watchlist_user_id   ON public.watchlist(user_id);
CREATE INDEX IF NOT EXISTS idx_watchlist_folder_id ON public.watchlist(folder_id);
...
CREATE TABLE IF NOT EXISTS public.certificates (
    ...
    issued_by          TEXT NOT NULL DEFAULT 'Other'
                         CHECK (issued_by IN ('GIA','AGS','IGI','GRS','GIT','GGTL','Gbelin','Other')),
    issued_date        DATE,
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_certificates_gem_id    ON public.certificates(gem_id);
...
DROP POLICY IF EXISTS "certificates_insert_seller" ON public.certificates;
...
DROP POLICY IF EXISTS "certificates_delete_pending" ON public.certificates;
...
CREATE TABLE IF NOT EXISTS public.reviews (
    ...
    comment        TEXT,
    media_urls     TEXT[] NOT NULL DEFAULT '{}' CHECK (COALESCE(array_length(media_urls, 1), 0) <= 6),
...
-- Called from frontend via supabase.rpc('place_bid', {...})
...
-- Called from frontend via supabase.rpc('buy_now', {...})
-- 
...
BEGIN
    ...
    -- supabase_realtime is the default Supabase Realtime publication.
    IF NOT EXISTS (
...
-- 
-- GRANTS  Ensure authenticated role has baseline access
-- 
...
-- ==================== BEGIN 003_add_certification_body.sql ====================
...
-- ==================== BEGIN 004_reviews_time_window_and_edit_limit.sql ====================
...
CREATE POLICY "reviews_update_limited" ON public.reviews
    ...
    USING (auth.uid() = reviewer_id AND edit_count < 3)
    WITH CHECK (auth.uid() = reviewer_id);
-- ==================== BEGIN 005_review_reports_moderation.sql ====================
...
-- ==================== BEGIN 006_reviews_media_uploads.sql ====================
...
-- ==================== BEGIN 007_virtual_wallet_second_chance.sql ====================
-- 
-- TABLE 11  wallets
...
-- ==================== BEGIN 008_review_post_sequence.sql ====================
...
-- ==================== BEGIN 010_wallet_gems_and_withdrawals.sql ====================
